...
if [[ ! -x "$LB_BINARY" ]]; then
    echo -e "${RED}❌ Load balancer binary '$LB_BINARY' not found or not executable.${NC}"
    echo "💡 Make sure to build it first: go build -o loadbalancer"
    exit 1
fi
...
if [ $missing_backends -gt 0 ]; then
    ...
    echo "💡 Start test backends with: go run backend.go --type=controllable --port=XXXX"
fi

# Start load balancer
echo -e "\n${GREEN}✅ Starting load balancer on port $LB_PORT...${NC}"
$LB_BINARY > lb_test.log 2>&1 &
LB_PID=$!
echo -e "${BLUE}🔁 Load balancer PID: $LB_PID${NC}"

# Wait for load balancer to be ready
if ! wait_for_service "http://localhost:$LB_PORT" 10; then
    echo -e "${RED}❌ Load balancer failed to start properly${NC}"
    echo "📋 Last few lines from lb_test.log:"
    tail -n 5 lb_test.log
    exit 1
fi
...
echo -e "\n${BLUE}🚀 Load balancer is running!${NC}"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    echo "📋 Load balancer logs saved to: lb_test.log"
    echo "💡 Use 'tail -f lb_test.log' to follow logs in real-time"
    
else
    echo -e "${BLUE}📱 Load balancer is running in background...${NC}"
